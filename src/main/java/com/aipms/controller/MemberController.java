...
public class MemberController {
    ...
    public ResponseEntity<?> updateMember(
        ...
        memberService.modifyMember(id, dto);
    ...
    public ResponseEntity<String> deactivateMember(@PathVariable Long memberId) {
        try {
            memberService.deactivateMember(memberId); // 서비스에서 status만 INACTIVE로 바꿈
    ...
    public ResponseEntity<String> activateMember(@PathVariable Long memberId) {
        try {
            memberService.activateMember(memberId); // 서비스에서 status만 INACTIVE로 바꿈
    ...
    public String updateMemberInfo(
        ...
        log.info("📥 업로드된 파일: {}", profileImageFile != null ? profileImageFile.getOriginalFilename() : "NULL");

        memberDto.setProfileImageFile(profileImageFile);

        Long memberId = memberDto.getMemberId();
        if (memberId == null) {
            redirectAttributes.addFlashAttribute("error", "회원 ID가 필요합니다.");
